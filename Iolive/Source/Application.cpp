...
namespace Iolive {
    ...
    void Application::OnNetworkUpdate()
    {
		if (m_client)
		{
			m_client->Update();
			if (m_in_room)
			{
				if (m_UserModel.GetModel2D())
				{
					// If this is a receive client, this is empty and does not send.
					ParameterBinding binding = m_UserModel.GetModel2D()->GetBindedParameter();
					std::map<int, float> parameters;
					for (const auto& [index, value] : binding)
					{
						parameters[index] = *value;
					}
					m_client->SendParams(parameters);
				}
			}
		}
    }
    ...
    void Application::OnNetworkRequest(std::string address, uint16_t port, ionet::ClientMode client_mode)
    {
        // std::unique_ptr<ionet::IonetClient> client;
        switch (client_mode) {
        ...
        m_client->SetSendRoomsHandler(
            [this](std::vector<ionet::RoomId> room_ids) {
                this->m_rooms = room_ids;
            });
        m_client->SetJoinRoomAcceptedHandler(
            [this](ionet::RoomId room_id) {
                this->m_in_room = true;
                this->m_current_room = room_id;
            });
        m_client->SetJoinRoomRejectedHandler(
            [this](ionet::RoomId room_id) {
    ...
    void Application::BindDefaultParametersWithNetwork()
    {
        ...
        // check is parameter exist?, then bind it
        if (paramIndex.ParamAngleX > -1)
            model->SetParameterBindingAt(paramIndex.ParamAngleX, &ModelParams[paramIndex.ParamAngleX]);
        if (paramIndex.ParamAngleY > -1)
            model->SetParameterBindingAt(paramIndex.ParamAngleY, &ModelParams[paramIndex.ParamAngleY]);
        if (paramIndex.ParamAngleZ > -1)
            model->SetParameterBindingAt(paramIndex.ParamAngleZ, &ModelParams[paramIndex.ParamAngleZ]);
        if (paramIndex.ParamBodyAngleX > -1)
            model->SetParameterBindingAt(paramIndex.ParamBodyAngleX, &ModelParams[paramIndex.ParamBodyAngleX]);
        if (paramIndex.ParamBodyAngleY > -1)
            model->SetParameterBindingAt(paramIndex.ParamBodyAngleY, &ModelParams[paramIndex.ParamBodyAngleY]);
        if (paramIndex.ParamBodyAngleZ > -1)
            model->SetParameterBindingAt(paramIndex.ParamBodyAngleZ, &ModelParams[paramIndex.ParamBodyAngleZ]);
        if (paramIndex.ParamEyeLOpen > -1)
            model->SetParameterBindingAt(paramIndex.ParamEyeLOpen, &ModelParams[paramIndex.ParamEyeLOpen]);
        if (paramIndex.ParamEyeROpen > -1)
            model->SetParameterBindingAt(paramIndex.ParamEyeROpen, &ModelParams[paramIndex.ParamEyeROpen]);
        if (paramIndex.ParamEyeLSmile > -1)
            model->SetParameterBindingAt(paramIndex.ParamEyeLSmile, &ModelParams[paramIndex.ParamEyeLSmile]);
        if (paramIndex.ParamEyeRSmile > -1)
            model->SetParameterBindingAt(paramIndex.ParamEyeRSmile, &ModelParams[paramIndex.ParamEyeRSmile]);
        if (paramIndex.ParamEyeForm > -1)
            model->SetParameterBindingAt(paramIndex.ParamEyeForm, &ModelParams[paramIndex.ParamEyeForm]);
        if (paramIndex.ParamEyeBallX > -1)
            model->SetParameterBindingAt(paramIndex.ParamEyeBallX, &ModelParams[paramIndex.ParamEyeBallX]);
        if (paramIndex.ParamEyeBallY > -1)
            model->SetParameterBindingAt(paramIndex.ParamEyeBallY, &ModelParams[paramIndex.ParamEyeBallY]);
        if (paramIndex.ParamMouthOpenY > -1)
            model->SetParameterBindingAt(paramIndex.ParamMouthOpenY, &ModelParams[paramIndex.ParamMouthOpenY]);
        if (paramIndex.ParamMouthForm > -1)
            model->SetParameterBindingAt(paramIndex.ParamMouthForm, &ModelParams[paramIndex.ParamMouthForm]);
        if (paramIndex.ParamBrowLY > -1)
            model->SetParameterBindingAt(paramIndex.ParamBrowLY, &ModelParams[paramIndex.ParamBrowLY]);
        if (paramIndex.ParamBrowRY > -1)
            model->SetParameterBindingAt(paramIndex.ParamBrowRY, &ModelParams[paramIndex.ParamBrowRY]);
        if (paramIndex.ParamBrowLForm > -1)
            model->SetParameterBindingAt(paramIndex.ParamBrowLForm, &ModelParams[paramIndex.ParamBrowLForm]);
        if (paramIndex.ParamBrowRForm > -1)
            model->SetParameterBindingAt(paramIndex.ParamBrowRForm, &ModelParams[paramIndex.ParamBrowRForm]);
        if (paramIndex.ParamBrowLAngle > -1)
            model->SetParameterBindingAt(paramIndex.ParamBrowLAngle, &ModelParams[paramIndex.ParamBrowLAngle]);
        if (paramIndex.ParamBrowRAngle > -1)
            model->SetParameterBindingAt(paramIndex.ParamBrowRAngle, &ModelParams[paramIndex.ParamBrowRAngle]);
